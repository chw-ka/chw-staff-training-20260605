...
| Cursor Agent | 下達書面語指令、Approve tool calls |
| Google OAuth | 授權 Agent 讀寫**你指定**的 Drive |

## 課前準備（每位老師）

1. 跟 [`handouts/06-google-drive-mcp-setup.md`](../handouts/06-google-drive-mcp-setup.md) 完成 OAuth + MCP 設定
2. 在 Google Drive 建立 demo folder **「CHW_Training_垃圾崗」**
3. 將 `samples/` 裡面 4 個亂碼檔**上傳**到該 folder
...
4. **（10 min）** 全班貼 Prompt → 逐次 **Approve** MCP tool calls → 看 Drive 變化
5. **（5 min）** 討論：與本地執 file 有什麼分別？校內 Shared Drive 如何用？

## 安全提醒

- OAuth 只授權**你個人** Google 帳號
- 建議只用 **CHW_Training_*** 測試 folder，不要叫 Agent 整理整個 My Drive
- 所有 move/rename 都會在 Approve 前顯示 — 看清楚再按
