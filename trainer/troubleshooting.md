...
| 搵唔到 Gemini 欄位 | Settings → Models → 搜尋 Google / Gemini |
| DeepSeek 填邊度？ | OpenAI API Key 欄 + Override URL（見 01 設定指南） |
| 無 Verify 按鈕 | 更新 Cursor；手動喺 Agent 試一句 |
...
| `redirect_uri_mismatch` | OAuth client 必須係 **Desktop app** |
| Access blocked | OAuth consent 加 Test user |
| listFolder 搵唔到 folder | folder 名：`CHW_Training_垃圾崗` |
| Agent 唔用 MCP | Prompt 寫明「用 Google Drive MCP」 |
| 用邊個 model？ | **deepseek-v4-flash** |
...
| 剩餘時間 | 建議 |
|----------|------|
| 少 5 分鐘 | 活動三展示 template，唔 live 生圖 |
